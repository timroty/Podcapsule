...
export function PodcastList() {
  ...
  const handleDelete = useCallback(async (index: number, podcast: any) => {
    try {
      let access_token = (await supabase.auth.getSession()).data.session?.access_token ?? "";
      let successfullyDeleted = await DeleteUserPodcast(access_token, podcast.id);
  ...
  return (
    <>
      {podcasts.map((podcast: any, index) => {
        return (
          <div key={index}>
            { index !== 0 ? <div className="border-t border-gray-800" /> : <div /> }
            <div className="grid grid-cols-12 gap-4 h-32">
              <div className="flex items-center justify-center col-span-3 sm:col-span-3 md:col-span-2">
                <img src={podcast.image_url} alt={`${podcast.title} cover image.`} className="w-24 rounded" />
              </div>
