import { User, FavoritedPodcastEpisode } from "../types/DatabaseTypes";
...
export async function GetUserRandomFavoritedPodcast(
  userId: number,
): Promise<number> {
  const supabase = createClient(
    process.env.SUPABASE_PROJECT_URL,
...
export async function GetUserRandomFavoritedPodcastEpisode(
  favoritedPodcastId: number,
): Promise<FavoritedPodcastEpisode> {
  const supabase = createClient(
    process.env.SUPABASE_PROJECT_URL,
    process.env.SUPABASE_PROJECT_SECRET,
  );
  ...
  const result: FavoritedPodcastEpisode = data;

  return result;
}

export async function GetPodcastTitle(
  favoritedPodcastId: number,
): Promise<string> {
  const supabase = createClient(
    process.env.SUPABASE_PROJECT_URL,
  ...
  const result: string = data.Title;

  return result;
}

export async function UpdateUserRandomFavoritedPodcastEpisodeExist(
...
export async function UpdateUserLastSyncDate(
  userId: number,
  syncDate: string,
): Promise<void> {
  const supabase = createClient(
...
export async function RemoveUserRandomFavoritedPodcastEpisode(
  id: number,
): Promise<void> {
  const supabase = createClient(
    process.env.SUPABASE_PROJECT_URL,
